...
**Chat assistant (built, ahead of the original roadmap):** `/ai` — a free-tier-only OpenRouter streaming Q&A assistant over Finance/Goals/Career/Skills/Life Compass data. **Write capability added 2026-07-20, explicitly requested — suggest-only, same boundary as the other two AI surfaces below**: a message that looks like a write request (`looksLikeActionRequest()`) routes to a separate buffered endpoint that returns a validated, structured proposal instead of a streamed answer; the user reviews it as an inline approve/reject card and only an explicit approve executes the corresponding normal `useLifeStore` action (`addTransaction`, `updateGoal`, etc.) — the AI itself never writes. Scoped narrowly (confirmed via `AskUserQuestion`): create + update only, no delete/archive of anything, ever; and Reflections/Wins/Letters/Weekly Review stay completely off-limits — those are the user's own words, not something for an AI to author. Full status, architecture, and privacy verification: `docs/features/ai-assistant.md`.
