...
| NFR-REL-04 | (2026-07-20) Stored state pada versi/skema lama tidak pernah di-reject total — `migrateState()` di `store.js` selalu backfill field baru dari default dan mempertahankan field lama, tidak pernah `return null` hanya karena version mismatch. Lihat `docs/features/supabase-sync.md` "Reliability hardening". |
| NFR-REL-05 | (2026-07-20) Push ke Supabase yang gagal tidak pernah silent — `pendingSync` flag persisted ke localStorage, notifikasi "Gagal sync ke server" ditampilkan, retry otomatis saat app dibuka lagi (`hydrate()`) dan saat browser online kembali (`window` `online` event, `providers.jsx`) |
| NFR-REL-06 | (2026-07-20) Saat Supabase configured, hydrate() tidak pernah membandingkan timestamp untuk memutuskan sumber data — device dengan `pendingSync` push perubahannya, device tanpa `pendingSync` mempercayai Supabase sepenuhnya. Menggantikan heuristik "timestamp terbaru menang" yang terbukti tidak aman (device tanpa data asli bisa "menang" hanya karena tidak punya timestamp lokal untuk dibandingkan) |
| NFR-REL-07 | (2026-07-20) Build tanpa env var Supabase (`NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY`) menampilkan notifikasi persisten ("Sync ke cloud tidak aktif") alih-alih diam-diam berjalan local-only |
