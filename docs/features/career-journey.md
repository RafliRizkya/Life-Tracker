...
## Gamified single-track skill map (2026-07-19)

Source: `docs/prompt/update-ui-career.md`. The dual-lane layout above was itself replaced — showing both tracks side by side still read as cluttered/corporate. Shipped:

- **Track toggle** (segmented control, "Jejak Profesional" / "Milestone & Pencapaian") above the trail — only one track renders at a time. `track` derivation logic unchanged (still `type === "experience"` → `"experience"`).
- **Winding skill-tree path**: `CareerTrail.jsx` positions nodes in a fixed 3-anchor zigzag (left/center/right, `X_ANCHORS = [22, 50, 78]` as % of trail width) and draws a smooth S-curve through them (`buildPath()` — per-segment cubic bezier with vertical-tangent control points, the standard "flowchart curve" trick). Path renders in two layers: a muted dashed base (all points) and a solid forest-green overlay up through the last non-`planned` node — reads as "unlocked path so far, locked ahead."
- **Game nodes**: `TrailCard.jsx` now renders a 56px circular icon button per milestone (not a text card) — solid fill for completed, pulsing glow ring for `in_progress` ("you are here"), dashed hollow for `planned` ("locked"), small corner dot for `ongoing`. Title + date sit below the node, always visible; everything else (org, location, description, skills, evidence) moved fully into the click-through detail drawer — an intentional decluttering, not a regression, per the brief's "not a stiff timeline" goal.
- **Legend simplified**: dropped the "solid block = experience / badge = milestone" lines (moot now that only one track shows at a time); kept the Completed/In progress/Planned status legend, since that now maps directly to node fill state.

**Bug caught during verification, worth remembering**: the first pass centered nodes with a raw `style={{ transform: "translate(-50%, -50%)" }}` on a `motion.li`. Framer Motion silently ignores a manually-set `transform` string on `motion.*` components — it manages `transform` itself for `initial`/`animate`/`whileHover` — so the centering never applied and nodes rendered anchored at their top-left corner instead of centered on the path point (visually: the curve looked like it skipped every other node). Fix: use Framer's own `x`/`y` style motion values (`style={{ x: "-50%", y: "-50%" }}`), which compose correctly with scale/opacity animations. Any future absolutely-positioned `motion.*` element in this codebase needing a translate-based offset must use `x`/`y`, never a raw `transform` string.

Verified: production build clean, Playwright-checked both tracks (path visually threads every node after the fix, confirmed via bounding-box + SVG `d`-attribute inspection, not just a screenshot glance), node click still opens the existing detail drawer, mobile 390px, dark mode, no console errors introduced.
